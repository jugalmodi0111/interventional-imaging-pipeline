...
- **Inference safety:** temporal voting over a cine window — `src/eval/temporal_vote.py` (~~was `src/serve/temporal_vote.py`~~; deleted from `src/serve/` 2026-08-13 with the video path, restored 2026-08-16 under `src/eval/` for **offline** cine scoring only — not reachable from a live request) — recovers missed frames + drops one-frame flicker; triage/abstention (`src/serve/stenosis_triage.py`) defers OOD / low-confidence cases to a human while the model is sub-floor.
