...
OFFLINE EVALUATION ONLY. This lived at src/serve/temporal_vote.py until 2026-08-13, when the
serve video path was deleted (2026-08-03 audit P3): Model One screens a SINGLE STILL FRAME,
so nothing in the request path may aggregate over a clip. Scoring a detector over raw cine
offline is a different concern -- it is how the per-video sensitivity number in
STAGE2_PHASE1_POA P1.1b is computed, which is the evidence behind the proposed gate reframe
(per-video sensitivity + abstention instead of per-frame F1). Restored here 2026-08-16, under
src/eval/ rather than src/serve/, so the boundary is structural: importing it cannot put a
cine aggregate back into a served verdict.
